<template><div><p><a name="qARQK"></a></p>
<h2 id="ingress和ingress控制器介绍" tabindex="-1"><a class="header-anchor" href="#ingress和ingress控制器介绍" aria-hidden="true">#</a> Ingress和Ingress控制器介绍</h2>
...
<p>所以k8s提供了另一种方式，使用Ingress和Ingress控制器来对外暴露服务，Ingress控制器作为统一的流量入口，管理内部各种必要的服务，并通过Ingress资源来描述如何区分流量及内部的路由逻辑。有了Ingress和Ingress控制器，就可以通过定义路由流量的规则来实现服务发布，而无需创建NodePort或LoadBalancer类型的Service，并且流量也会由Ingress控制器直达Pod，不需要再由Service转发。</p>
<div class="hint-container info">
<p class="hint-container-title">相关信息</p>
<p>Ingress资源就是基于HTTP虚拟主机或URL路径的流量转发规则（类似于nginx中的虚拟主机定义或location转发规则定义），它把需要暴露给集群外的每个Service对象，映射为Ingress控制器上的一个虚拟主机或某虚拟主机的一个URL路径。</p>
</div>
<p>如下图所示：</p>
<figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675821066302-e7f2e7a2-7032-4d52-b186-470babac7d1b.png#averageHue=%23f1dacd&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=452&amp;id=u548d2323&amp;name=image.png&amp;originHeight=452&amp;originWidth=611&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=27821&amp;status=done&amp;style=none&amp;taskId=uc2dbc2b0-ffb7-4d90-9d7b-c9f3fed1582&amp;title=&amp;width=611" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
<p>Ingress官方文档：<a href="https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress/" target="_blank" rel="noopener noreferrer">https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress/<ExternalLinkIcon/></a><br />Ingress控制器官方文档：<a href="https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/" target="_blank" rel="noopener noreferrer">https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/<ExternalLinkIcon/></a></p>
<p><a name="JgTFh"></a></p>
<h3 id="ingress控制器" tabindex="-1"><a class="header-anchor" href="#ingress控制器" aria-hidden="true">#</a> Ingress控制器</h3>
<p>但Ingress资源本身只是一组路由规则定义，这些规则想要真正的生效还需要借助其它功能的辅助，例如监听某套接字、根据路由规则匹配机制将客户端请求进行转发等。实现这些功能的组件就是Ingress控制器(Ingress Controller)。Ingress Controller是Kubernetes的一个附件需要单独部署。</p>
<p><a name="AQ8zJ"></a></p>
<h2 id="ingress-controller部署" tabindex="-1"><a class="header-anchor" href="#ingress-controller部署" aria-hidden="true">#</a> Ingress Controller部署</h2>
<p>目前可选择使用的Ingress控制器有很多，可以参考官方介绍：<a href="https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/" target="_blank" rel="noopener noreferrer">https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/<ExternalLinkIcon/></a> ，下面以nginx Ingress控制器为例进行部署。</p>
<p>nginx Ingress控制器github地址：<a href="https://github.com/kubernetes/ingress-nginx" target="_blank" rel="noopener noreferrer">https://github.com/kubernetes/ingress-nginx<ExternalLinkIcon/></a><br />nginx Ingress控制器官方文档：<a href="https://kubernetes.github.io/ingress-nginx/" target="_blank" rel="noopener noreferrer">https://kubernetes.github.io/ingress-nginx/<ExternalLinkIcon/></a></p>
<p>** 常用的的Ingress控制器部署方式有两种： **</p>
<p><a name="Cd3df"></a></p>
<h3 id="_1-以deployment方式部署ingress控制器pod资源" tabindex="-1"><a class="header-anchor" href="#_1-以deployment方式部署ingress控制器pod资源" aria-hidden="true">#</a> 1. 以Deployment方式部署Ingress控制器Pod资源</h3>
<p>通过NodePort或LoadBalancer类型的Service或者通过拥有外部IP地址（externalIP）的Service对象为其接入集群外部的客户端请求流量。<br />这意味着，在生产环境以这种方式部署一个Ingress控制器时，必须在其前端定义一个负载均衡器，这个负载均衡器可以是LoadBalancer类型的Service，也可以是用户自行管理的负载均衡器。</p>
<figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675822367503-e4adce9e-bfca-4bb3-9b2f-549eac4f60ed.png#averageHue=%23f8f2ef&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=614&amp;id=u440a438b&amp;name=image.png&amp;originHeight=614&amp;originWidth=733&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=45763&amp;status=done&amp;style=none&amp;taskId=uec696fd8-906b-4a0d-8682-a98c7ffc3f4&amp;title=&amp;width=733" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
<p><a name="PTwIB"></a></p>
<h3 id="_2-以daemonset方式部署ingress控制器" tabindex="-1"><a class="header-anchor" href="#_2-以daemonset方式部署ingress控制器" aria-hidden="true">#</a> 2.以DaemonSet方式部署Ingress控制器</h3>
<p>Pod资源Ingress控制器的各Pod分别以单一实例的方式运行在集群的所有节点或部分专用节点之上，并配置这些Pod对象以hostPort或hostNetwork的方式在当前节点接入外部流量。在这种方式下，前端还是需要一个负载均衡器，作为客户端流量的统一入口，然后转发给Ingress控制器Pod</p>
<figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675822438502-f95a9c45-e281-47e8-b65f-fce5abecd40a.png#averageHue=%23f6f6f2&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=486&amp;id=u9ac953d6&amp;name=image.png&amp;originHeight=486&amp;originWidth=637&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=28055&amp;status=done&amp;style=none&amp;taskId=ud1143d45-3b31-4158-96f6-1775ca6a71b&amp;title=&amp;width=637" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
<p>在nginx Ingress控制器官方提供的部署文件中，默认使用第一种方式，使用Deployment+NodePort Service来部署。</p>
<p><a name="zJrih"></a></p>
<h3 id="_3-deployment方式部署" tabindex="-1"><a class="header-anchor" href="#_3-deployment方式部署" aria-hidden="true">#</a> 3. Deployment方式部署</h3>
<p>选定好版本，下载对应的部署文件</p>
<figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675833096495-64fbd3a8-e6ab-4eeb-9c19-063d0909ac07.png#averageHue=%23141a21&amp;clientId=ua68a97d2-e0ab-4&amp;from=paste&amp;height=613&amp;id=u1943c99c&amp;name=image.png&amp;originHeight=613&amp;originWidth=679&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=42264&amp;status=done&amp;style=none&amp;taskId=u507f2dba-06ee-406d-ac54-eb7941f0218&amp;title=&amp;width=679" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>kubectl apply <span class="token parameter variable">-f</span> deploy.yaml
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>NAME                                        READY   STATUS      RESTARTS   AGE
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>默认情况下，ingress-nginx-controller只有一个副本，可以按需调整</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>kubectl scale <span class="token parameter variable">-n</span> ingress-nginx deployment ingress-nginx-controller <span class="token parameter variable">--replicas</span><span class="token operator">=</span><span class="token number">3</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在负载均衡器中添加ingress-nginx-controller后端，以haproxy为例</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">cat</span> /etc/haproxy/harpoxy.cfg
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676361150547-2ea0c07b-1974-4351-9488-f601238b0a2c.png#averageHue=%230f111a&amp;clientId=uf152a37f-4e38-4&amp;from=paste&amp;height=288&amp;id=u7115d88f&amp;name=image.png&amp;originHeight=288&amp;originWidth=693&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=21240&amp;status=done&amp;style=none&amp;taskId=ud6ec3785-d612-46da-be12-99c1e6569f7&amp;title=&amp;width=693" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
<p><a name="bH2vb"></a></p>
<h3 id="_4-daemonset方式部署" tabindex="-1"><a class="header-anchor" href="#_4-daemonset方式部署" aria-hidden="true">#</a> 4.DaemonSet方式部署</h3>
<p>对前面的<strong>deploy.yaml</strong>进行修改，主要修改3个配置</p>
<ol>
<li>删除掉ingress-ingress-controller Service资源定义</li>
<li>将Deployment修改未DaemonSet</li>
<li>配置Pod使用<strong>hostNetwork</strong>和<strong>hostPID</strong></li>
</ol>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>同样的，也需要在负载均衡器中添加ingress-nginx-controller后端，以haproxy为例</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">cat</span> /etc/haproxy/harpoxy.cfg
<span class="token comment">#################################</span>
listen ingress-nginx-controller-80
       <span class="token builtin class-name">bind</span> <span class="token number">10.1</span>.0.6:80
...
listen ingress-nginx-controller-443
       ...
       server ingress-controller-server1 <span class="token number">10.1</span>.0.31:30021 check inter <span class="token number">2000</span> fall <span class="token number">3</span> rise <span class="token number">5</span>
       server ingress-controller-server2 <span class="token number">10.1</span>.0.32:30021 check inter <span class="token number">2000</span> fall <span class="token number">3</span> rise <span class="token number">5</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="Bd8wp"></a></p>
<h2 id="_3-ingress示例" tabindex="-1"><a class="header-anchor" href="#_3-ingress示例" aria-hidden="true">#</a> 3.Ingress示例</h2>
<p><a name="bIkUq"></a></p>
<h3 id="_3-1-ingress资源规范" tabindex="-1"><a class="header-anchor" href="#_3-1-ingress资源规范" aria-hidden="true">#</a> 3.1 Ingress资源规范</h3>
<p>Ingress资源的可用字段和含义如下：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>apiVersion: networkking.k8s.io/v1
kind: Ingress
metadata:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="hgQMr"></a></p>
<h3 id="_3-2-单域名访问示例" tabindex="-1"><a class="header-anchor" href="#_3-2-单域名访问示例" aria-hidden="true">#</a> 3.2 单域名访问示例</h3>
<p>先创建一下资源，用于测试Ingress功能，部署文件如下，包含两个tomcat-pod和两个对应的Svc<br /></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>pod创建成功后 到里面创建一个测试页面</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">-</span><span class="token punctuation">-</span>tomcat<span class="token punctuation">-</span>app1<span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span><span class="token punctuation">---</span>
...
tomcat app2 ingress
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="sPXIg"></a></p>
<h4 id="_3-2-1-单域名-ingress-资源" tabindex="-1"><a class="header-anchor" href="#_3-2-1-单域名-ingress-资源" aria-hidden="true">#</a> 3.2.1 单域名 ingress 资源</h4>
<p><a href="http://xn--www-zr0fl02s60m.app1.com" target="_blank" rel="noopener noreferrer">将访问www.app1.com<ExternalLinkIcon/></a> 域名的流量转发至tomcat-app1-svc，Ingress部署文件如下：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> networking.k8s.io/v1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="fSKZN"></a></p>
<h4 id="_3-2-2-多域名-ingress-资源" tabindex="-1"><a class="header-anchor" href="#_3-2-2-多域名-ingress-资源" aria-hidden="true">#</a> 3.2.2 多域名 ingress 资源</h4>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> networking.k8s.io/v1
<span class="token key atrule">kind</span><span class="token punctuation">:</span> Ingress
<span class="token key atrule">metadata</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="Vuw1T"></a></p>
<h4 id="_3-2-3-配置haproxy-keepalived-实现负载均衡" tabindex="-1"><a class="header-anchor" href="#_3-2-3-配置haproxy-keepalived-实现负载均衡" aria-hidden="true">#</a> 3.2.3 配置Haproxy+ keepalived 实现负载均衡</h4>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">apt</span> <span class="token function">install</span> keepalived haproxy <span class="token parameter variable">-y</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>keepalived 配置文件如下：</p>
...
vrrp_instance VI_1 <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Haproxy 配置使用VIP代理 <strong>ingress controler service</strong> <br />Haproxy配置如下： 两个节点一样</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>listen k8s-xin-ingress-80
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676441359881-2d49ba70-9af5-4d2a-b4c4-e5bba389aa3c.png#averageHue=%23a3ecb5&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=89&amp;id=u5a4cfc1b&amp;name=image.png&amp;originHeight=89&amp;originWidth=513&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=5207&amp;status=done&amp;style=none&amp;taskId=u3b0852d6-7897-4d1f-af17-af1084fc021&amp;title=&amp;width=513" alt="image.png" loading="lazy"><br /><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676441343917-040c8c97-7dc3-4f51-ac74-3671a2f8651d.png#averageHue=%23a3ecb5&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=78&amp;id=uf32278bc&amp;name=image.png&amp;originHeight=78&amp;originWidth=533&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=5270&amp;status=done&amp;style=none&amp;taskId=ucf5375df-4624-4d66-bbcb-e0eaea85d9b&amp;title=&amp;width=533" alt="image.png" loading="lazy"></p>
<p><a name="IyYla"></a></p>
<h3 id="_3-3-ingress-配置tls" tabindex="-1"><a class="header-anchor" href="#_3-3-ingress-配置tls" aria-hidden="true">#</a> 3.3 Ingress 配置TLS</h3>
<p><a name="XuS8z"></a></p>
<h4 id="_3-3-1-创建证书" tabindex="-1"><a class="header-anchor" href="#_3-3-1-创建证书" aria-hidden="true">#</a> 3.3.1 创建证书</h4>
<p><a href="http://xn--www-ps1eowg26al27i.myapp1.com" target="_blank" rel="noopener noreferrer">首先准备www.myapp1.com<ExternalLinkIcon/></a> 域名的证书，然后将证书保存为Secret</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mkdir</span> ingress-cert <span class="token operator">&amp;&amp;</span> <span class="token builtin class-name">cd</span> ingress-cert
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>查看Secret</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>root@master01<span class="token punctuation">[</span><span class="token number">14</span>:41:14<span class="token punctuation">]</span>~/ingress-cert <span class="token comment">#:kubectl describe secrets cert-www.myapp1.com </span>
...
<h4 id="_3-3-2-ingress使用tls证书示例" tabindex="-1"><a class="header-anchor" href="#_3-3-2-ingress使用tls证书示例" aria-hidden="true">#</a> 3.3.2 Ingress使用TLS证书示例</h4>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> networking.k8s.io/v1
<span class="token key atrule">kind</span><span class="token punctuation">:</span> Ingress
<span class="token key atrule">metadata</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="m2IUC"></a></p>
<h4 id="_3-3-3-pod-中创建用于测试的目录和页面" tabindex="-1"><a class="header-anchor" href="#_3-3-3-pod-中创建用于测试的目录和页面" aria-hidden="true">#</a> 3.3.3 pod 中创建用于测试的目录和页面</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="oQtYJ"></a></p>
<h4 id="_3-3-4-访问测试" tabindex="-1"><a class="header-anchor" href="#_3-3-4-访问测试" aria-hidden="true">#</a> 3.3.4 访问测试</h4>
<p><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676444292477-d1741e62-31dd-4c06-8e05-ff85d8fb18e2.png#averageHue=%23a9e6a0&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=83&amp;id=u1073d74c&amp;name=image.png&amp;originHeight=83&amp;originWidth=534&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=5895&amp;status=done&amp;style=none&amp;taskId=u4a9dbb38-ba52-42b3-98d6-4d44f834c37&amp;title=&amp;width=534" alt="image.png" loading="lazy"><br /><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676444442373-03d6e130-fea0-4e91-8f83-7121b860019d.png#averageHue=%2393db95&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=82&amp;id=u6240276d&amp;name=image.png&amp;originHeight=82&amp;originWidth=522&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=5863&amp;status=done&amp;style=none&amp;taskId=u1ab1ba8e-2fae-461c-9797-0ef08af40c6&amp;title=&amp;width=522" alt="image.png" loading="lazy"></p>
<p><a name="xadme"></a></p>
<h3 id="_3-4-证书更新" tabindex="-1"><a class="header-anchor" href="#_3-4-证书更新" aria-hidden="true">#</a> 3.4 证书更新</h3>
<p>假设网站的https证书即将过期，在不影响业务的前提下，可以直接更新其引用的Secret中保存的证书来实现网站https证书更新。在生产环境需要提前做好计划，并选择合适时间执行。</p>
<p>首先重新签发一套证书</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>openssl genrsa <span class="token parameter variable">-out</span> www.myapp1.com-new.key <span class="token number">4096</span>
openssl req <span class="token parameter variable">-sha512</span> <span class="token parameter variable">-new</span> <span class="token punctuation">\</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>将证书和key的内容进行base64编码，然后编辑相应的Secret对象，修改tls.key和tls.crt的值为编码后的内容</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>root@master-01:~/resources/ingress-cert<span class="token comment"># base64 www.myapp1.com-new.key -w 0</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676445008460-e809acd8-60e2-4ef6-ac23-1faa36ddf2cc.png#averageHue=%2312151e&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=698&amp;id=uf571f9a8&amp;name=image.png&amp;originHeight=698&amp;originWidth=1887&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=266981&amp;status=done&amp;style=none&amp;taskId=u85c0173a-13ef-4387-bf95-bd584af672a&amp;title=&amp;width=1887" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
<p>和之前的访问结果进行对比，可以看到证书已经被更新</p>
</div></template>
