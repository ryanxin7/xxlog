<template><div><p>ceph集群配置、部署与运维</p>
<p><br><a href="http://docs.ceph.org.cn/rados/" target="_blank" rel="noopener noreferrer">http://docs.ceph.org.cn/rados/<ExternalLinkIcon/></a></p>
...
<p><strong>[global] <strong>#全局配置</strong>[osd]</strong> #osd专用配置，可以使用osd.N, 来表示某一个OSD专用配置，N为osd的编号，如0、2、1等，</p>
<p><br><strong>[mon]</strong> #mon专用配置，也可以使用mon.A来为某一个monitor节点做专用配置，其中A为该节点的名称，ceph-monitor-2、 ceph-monitor-1 等，使用命令ceph mon dump可以获取节点的名称、</p>
<p><br><strong>[client]</strong> #客户端专用配置.</p>
</div>
...
<br>
<figure><img src="http://cdn1.ryanxin.live/xxlog/1669691960330-9d8aa14f-ec4b-4d76-a6df-2a40bfc26a09.png" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>
...
<p><br>一般采用8+4机制，即8个数据块+4个冗余块，那么也就是12个数据块有8个数据块保存数据,有4个实现数据冗余，即1/3的磁盘空间用于数据冗余，比默认副本池的三倍冗余节省空间,但是不能出现大于一定数据块故障。</p>
<br>
...
<p><br>在客户端写入操作时，ceph使用CRUSH算法计算出与对象相对应的<strong>PG ID</strong>和<strong>primary OSD</strong></p>
<p><br>主OSD根据设置的副本数、对象名称、存储池名称和**集群运行图(cluster map)**计算出PG</p>
<p><br>的各辅助OSD，然后由OSD将数据再同步给辅助OSD.</p>
<p><strong>读取数据:</strong></p>
<p><br>1.客户端发送读请求，<strong>RADOS</strong> 将请求发送到主OSD.</p>
<p><br>2.主OSD从本地磁盘读取数据并返回数据，最终完成读请求。</p>
