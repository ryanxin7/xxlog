import{_ as n,V as s,W as a,X as e}from"./framework-8eb1d601.js";const i={},l=e(`<p>场景：集群内有固态和机械硬盘 如何让不重要的业务放到机械盘。 <br><img src="http://cdn1.ryanxin.live/xxlog/1670239925327-b0a6d81f-d49f-457f-ab71-329245a2c645.png" alt="image.png" loading="lazy"></p><p><strong>Ceph 集群中由 mon 服务器维护的的五种运行图</strong>:</p><ul><li>Monitor map 监视器运行图</li><li>OSD map OSD运行图 各个每隔6s汇报状态同时监控其他OSD的状态，超过20秒就会被踢出去</li><li>PG map PG运行图 （一个存储池有哪些pg）</li><li>Crush map (Controllers replication under scalable hashing) 可控的、可复制的、可伸缩的一致性hash算法。crush运行图，当新建存储池时会基于OSD map创建出新的PG组合列表用于存储数据</li><li>MDS map cephfs metadata运行图</li></ul><p>数据的访问：</p><p>obj--&gt;pg hash(oid)%pg=pgid 先将文件计算成一个hash值，这个数取pg数量的余数 最终得到分配到那个pg中<br>Obj--&gt;OSD crush 根据当前的mon运行图返回pg内的最新的OSD组合,数据即可开始往主的写然后往副本OSD同步</p><p><strong>crush 算法针对目的节点的选择</strong>:<br>目前有5种算法来实现节点的选择，包括Uniform、List、 Tree、 Straw、 Straw2, 早期版<br>本使用的是ceph项目的发起者发明的算法straw,目前已经发展到straw2版本。<br>抽签算法</p><h2 id="_9-1-pg与osd映射调整" tabindex="-1"><a class="header-anchor" href="#_9-1-pg与osd映射调整" aria-hidden="true">#</a> 9.1 PG与OSD映射调整</h2><p>默认情况下,crush算法自行对创建的pool中的PG分配OSD,但是可以手动基于权重设置 crush 算法分配数据的倾向性， 比如1T 的磁盘权重是1, 2T 的就是2, 推荐使用相同大小的设备。</p><p>调整的方法有两种：<br> 1. 调整weight值</p><ol start="2"><li>调整reweight值</li></ol><h3 id="_9-1-1-查看当前状态" tabindex="-1"><a class="header-anchor" href="#_9-1-1-查看当前状态" aria-hidden="true">#</a> 9.1.1 查看当前状态</h3><p><code>ceph osd df</code></p><p>基于存储空间<br>**weight **表示设备(device)的容量相对值，比如 1TB 对应1.00 ,那么500G的 OSD 的 weight 就应该是0.5, weight 是基于磁盘空间分配PG的数量，让crush算法尽可能往磁盘空间大的OSD多分配OSD。往磁盘空间小的OSD分配较少的OSD。</p><p>那个磁盘快满了调整一下释放资源<br>**Reweight **参数的目的是重新平衡 ceph的CRUSH算法随机分配的PG,默认的分配是概率上的均衡，即使OSD都是一 样的磁盘空间也会产生一些PG分布不均匀的情况， 此时可以通过调整reweight参数，让ceph集群立即重新平衡当前磁盘的PG,以达到数据均衡分布的目的，REWEIGHT 是PG已经分配完成，要在cepg集群重新平衡PG的分布。</p><h3 id="_9-1-2-修改weight并验证" tabindex="-1"><a class="header-anchor" href="#_9-1-2-修改weight并验证" aria-hidden="true">#</a> 9.1.2 修改WEIGHT并验证</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment">#修改某个指定ID的osd的权重 调整完立即生效</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-1-3-修改-reweight-并验证" tabindex="-1"><a class="header-anchor" href="#_9-1-3-修改-reweight-并验证" aria-hidden="true">#</a> 9.1.3 修改 REWEIGHT 并验证</h3><p>OSD 的 <strong>REWEIGHT</strong>的值默认为1 ,值可以调整，范围在0~1之间，值越低PG越小，如果调整了任何一个OSD的REWEIGHT值，那么OSD的PG会立即和其它OSD进行重新平衡，即数据的重新分配，用于当某个OSD的PG相对较多需要降低其PG 数量的场景。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># ceph osd reweight 9 0.6</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://cdn1.ryanxin.live/xxlog/1670241465024-9c13f208-cb26-418d-9bcf-3fab74f0a0b1.png" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><h2 id="_9-2-crush运行图管理" tabindex="-1"><a class="header-anchor" href="#_9-2-crush运行图管理" aria-hidden="true">#</a> 9.2 crush运行图管理</h2><p>通过工具将 ceph 的crush 运行图导出并进行编辑，然后导入</p><h3 id="_9-2-1-导出crush运行图" tabindex="-1"><a class="header-anchor" href="#_9-2-1-导出crush运行图" aria-hidden="true">#</a> 9.2.1 导出crush运行图</h3><p>注: 导出的 crush 运行图为二进制格式,无法通过文本编辑器直接打开，需要使用<strong>crushtool</strong>工具转换为文本格式后才能通过vim等文本编辑宫工具打开和编辑。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># mkdir /data/ceph -p </span>
<span class="token comment">#导出</span>
root@ceph-deploy:~<span class="token comment"># ceph osd getcrushmap -o /data/ceph/crushmap </span>
<span class="token number">67</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-2-2-将运行图转换为文本" tabindex="-1"><a class="header-anchor" href="#_9-2-2-将运行图转换为文本" aria-hidden="true">#</a> 9.2.2 将运行图转换为文本:</h3><p>导出的运行图不能直接编辑，需要转换为文本格式再进行查看与编辑</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># apt install ceph-base</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-2-3-编辑文本" tabindex="-1"><a class="header-anchor" href="#_9-2-3-编辑文本" aria-hidden="true">#</a> 9.2.3 编辑文本</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment">#自定义修改</span>
...
rule erasure-code <span class="token punctuation">{</span> <span class="token comment">#纠删码池的默认配置</span>
        ...
        step emit
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># rules </span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-2-3-将文本转换为crush格式" tabindex="-1"><a class="header-anchor" href="#_9-2-3-将文本转换为crush格式" aria-hidden="true">#</a> 9.2.3 将文本转换为crush格式</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># crushtool -c /data/ceph/crushmap.txt -o /data/ceph/newcrushmap</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_9-2-4-导入新的crush" tabindex="-1"><a class="header-anchor" href="#_9-2-4-导入新的crush" aria-hidden="true">#</a> 9.2.4 导入新的crush</h3><p>导入的运行图会立即覆盖原有的运行图并立即生效.</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># ceph osd setcrushmap -i /data/ceph/newcrushmap</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_9-2-5-验证crush运行图是否生效" tabindex="-1"><a class="header-anchor" href="#_9-2-5-验证crush运行图是否生效" aria-hidden="true">#</a> 9.2.5 验证crush运行图是否生效</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># ceph osd crush rule dump</span>
<span class="token punctuation">[</span>
    <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://cdn1.ryanxin.live/xxlog/1670291279565-a52a21c1-3232-44ef-a78c-5acf15757a98.png" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><h2 id="_9-3-crush数据分类管理" tabindex="-1"><a class="header-anchor" href="#_9-3-crush数据分类管理" aria-hidden="true">#</a> 9.3 crush数据分类管理</h2><p>Cephcrush 算法分配的PG的时候可以将PG分配到不同主机的OSD上，以实现以主机为单位的高可用，这也是默认机制，但是无法保证不同PG位于不同机柜或者机房的主机。</p><p>如果要实现基于机柜或者是更高级的IDC等方式的数据高可用，而且也不能实现A项目的数据在SSD,B项目的数据在机械磁盘，如果想要实现此功能则需要导出crush运行图并手动编辑，之后再导入并覆盖原有的crush运行图。</p><figure><img src="http://cdn1.ryanxin.live/xxlog/1670291396515-8d89ed36-5dd5-44df-a502-56b54f9a0c47.png" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><h3 id="_9-3-1-导出cursh运行图" tabindex="-1"><a class="header-anchor" href="#_9-3-1-导出cursh运行图" aria-hidden="true">#</a> 9.3.1 导出cursh运行图</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># mkdir /opt/ceph/</span>
root@ceph-deploy:~<span class="token comment"># ceph osd getcrushmap -o /opt/ceph/crushmap</span>
<span class="token number">68</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-2-将运行图转换为文本" tabindex="-1"><a class="header-anchor" href="#_9-3-2-将运行图转换为文本" aria-hidden="true">#</a> 9.3.2 将运行图转换为文本</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># crushtool -d /opt/ceph/crushmap &gt; /opt/ceph/crushmap.txt</span>
root@ceph-deploy:- <span class="token comment"># file /opt/ceph/crushmap.txt</span>
/opt/ceph/crushmap.txt: ASCII text
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-3-添加自定义配置" tabindex="-1"><a class="header-anchor" href="#_9-3-3-添加自定义配置" aria-hidden="true">#</a> 9.3.3 添加自定义配置</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># cat topt/ceph/crushmap.txt</span>
<span class="token comment"># begin crush map</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>rule data <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-4-转换为crush二进制格式" tabindex="-1"><a class="header-anchor" href="#_9-3-4-转换为crush二进制格式" aria-hidden="true">#</a> <br>9.3.4 转换为crush二进制格式</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># crushtool -C /opt/ceph/crushmap.txt -o /opt/ceph/newcrushmap</span>

</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-5-导入新的crush运行图" tabindex="-1"><a class="header-anchor" href="#_9-3-5-导入新的crush运行图" aria-hidden="true">#</a> 9.3.5 导入新的crush运行图</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># ceph osd setcrushmap -i /opt/ceph/crushmap</span>
<span class="token number">70</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-6-验证crush运行图是否生效" tabindex="-1"><a class="header-anchor" href="#_9-3-6-验证crush运行图是否生效" aria-hidden="true">#</a> 9.3.6 验证crush运行图是否生效</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># ceph osd crush rule dump</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_9-3-7-测试创建存储池" tabindex="-1"><a class="header-anchor" href="#_9-3-7-测试创建存储池" aria-hidden="true">#</a> 9.3.7 测试创建存储池</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>root@ceph-deploy:~<span class="token comment"># peph osd pool create magedu-ssdpool 32 32 magedu_ssd_rule</span>
pool <span class="token string">&#39;magedu-ssdpool&#39;</span> created

<span class="token comment">#指定存储池 rule 默认是defuse</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_9-3-8-验证pgp状态" tabindex="-1"><a class="header-anchor" href="#_9-3-8-验证pgp状态" aria-hidden="true">#</a> 9.3.8 验证pgp状态</h3><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>ceph pg ls-by-pool magedu-ssdpool <span class="token function">awk</span> <span class="token string">&#39;{print $1, $2,$15} &#39;</span>

</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://cdn1.ryanxin.live/xxlog/1670295323891-1d92b007-cefb-449a-a54d-38d366d62bf9.png" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>`,61),c=[l];function t(d,p){return s(),a("div",null,c)}const r=n(i,[["render",t],["__file","9. ceph-crush进阶.html.vue"]]);export{r as default};
