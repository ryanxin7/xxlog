...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_5-3-ceph用户" tabindex="-1"><a class="header-anchor" href="#_5-3-ceph用户" aria-hidden="true">#</a> 5.3 ceph用户</h2><p>用户是指个人(ceph管理者)或系统参与者**(MON/OSD/MDS)**.<br>通过创建用户，可以控制用户或哪个参与者能够访问ceph存储集群、以及可访问的存储池及存储池中的数据。<br>ceph支持多种类型的用户，但可管理的用户都属于client类型区分用户类型的原因在于, <strong>MON/OSD/MDS 等系统组件特使用cephx协议</strong>,但是它们为非客户端。</p>`,14),d={href:"http://TYPE.ID",target:"_blank",rel:"noopener noreferrer"},u=s("strong",null,"client. admin",-1),h=c(`<div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>cephaceph-dep Loy:~/ ceph-cluster$ <span class="token function">cat</span> ceph.Client.admin.key ring
<span class="token punctuation">[</span>client.admin<span class="token punctuation">]</span>
         key <span class="token operator">=</span> AQA3dhdhMd/UABAAr 2SNpJ+hcK1dD5 L2Hj <span class="token assign-left variable">5XMg</span><span class="token operator">==</span>
         caps mds <span class="token operator">=</span> <span class="token string">&quot;allow *&quot;</span>
         caps mgr <span class="token operator">=</span> <span class="token string">&quot;allow *&quot;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="_5-5-ceph用户管理" tabindex="-1"><a class="header-anchor" href="#_5-5-ceph用户管理" aria-hidden="true">#</a> 5.5 ceph用户管理</h2><p><br>用户管理功能可让 Ceph集群管理员能够直接在 Ceph集群中创建更新和删除用户。<br>在Ceph集群中创建或删除用户时，可能需要将密钥分发到客户端，以便将密钥添加到密钥环文件中<code>/etc/ceph/ceph.client.admin.keyring</code>，此文件中可以包含一个或 者多个用户认证信息，凡是拥有此文件的节点，将具备访问ceph的权限，而且可以使用其中任何一个账户的权限，此文件类似于linux系统的中的**/etc/passwd**文件.</p>`,17),v={href:"http://TYPE.ID",target:"_blank",rel:"noopener noreferrer"},k={href:"http://TYPE.ID",target:"_blank",rel:"noopener noreferrer"},m=s("br",null,null,-1),b=s("strong",null,"Ceph monitor (mon)",-1),g=s("strong",null,[n("Ceph OSD (osd)"),s("strong",null,"或"),n("Ceph元数据服务器(mds)")],-1),y=c(`<h3 id="_5-5-1-ceph-auth-add" tabindex="-1"><a class="header-anchor" href="#_5-5-1-ceph-auth-add" aria-hidden="true">#</a> 5.5.1 ceph auth add</h3><p>此命令是添加用户的规范方法。它会创建用户、生成密钥，并添加所有指定的能力。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token punctuation">[</span>ceph@ceph-deploy ceph-cluster<span class="token punctuation">]</span>$ ceph auth <span class="token parameter variable">-h</span>
...
key <span class="token operator">=</span> <span class="token assign-left variable">AQAUUchfjpMqGRAARV6h0ofdDEneuaRnxuHjoQ</span><span class="token operator">==</span>
     caps mon <span class="token operator">=</span> <span class="token string">&quot;allow r&quot;</span>
     caps osd <span class="token operator">=</span> <span class="token string">&quot;allow * pool=mypool&quot;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,35);function f(x,A){const e=o("ExternalLinkIcon");return p(),t("div",null,[r,s("p",null,[n("通过点号来分割用户类型和用户名，格式为"),s("strong",null,[s("a",d,[n("TYPE.ID"),a(e)])]),n(",例如"),u,n(" 通常容器和服务器使用client类型")]),h,s("blockquote",null,[s("p",null,[n("注意: TYPEID 表示法针对用户采用"),s("strong",null,[s("a",v,[n("TYPE.ID"),a(e)])]),n("表示法 例如osd.0指定是osd类并且ID为0的用户(节点), client.admin是client类型的用户，其ID为admin, 另请注意，每个项包含一个 key=xxx项，以及一个或多个caps项。 可以结合使用-0文件名选项和ceph auth list 将输出保存到某个文件, [ceph@ceph-deploy ceph-cluster]$ ceph auth list -o 123.key")])]),s("p",null,[n("添加一个用户会创建用户名("),s("strong",null,[s("a",k,[n("TYPE.ID"),a(e)])]),n("). 机密密钥，以及包含在命令中用于创建该用户的所有能力,用户可使用其密钥向Ceph 存储集群进行身份验证。"),m,n("用户的能力授予该用户在"),b,n("、"),g,n(" 上进行读取、写入或执行的能力,可以使用以下几个命令来添加用户:")]),y])}const _=i(l,[["render",f],["__file","5. CephX认证机制.html.vue"]]);export{_ as default};
