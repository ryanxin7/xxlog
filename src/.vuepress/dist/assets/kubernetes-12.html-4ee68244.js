import{_ as i,V as p,W as l,Z as s,a0 as n,Y as e,$ as t,C as c}from"./framework-7e159c24.js";const o={},r=t('<p><a name="qARQK"></a></p><h2 id="ingress和ingress控制器介绍" tabindex="-1"><a class="header-anchor" href="#ingress和ingress控制器介绍" aria-hidden="true">#</a> Ingress和Ingress控制器介绍</h2><p>在k8s中将一个服务暴露出去通常会使用NodePort或LoadBalancer类型的Service，但随着服务数量的增多，使用NodePort会存在一些问题，可用作NodePort的端口是一个有限的范围，不容易记忆，不好管理。另外， 如果在公有云使用LoadBalancer类型的Service上会产生额外的成本。</p><p>所以k8s提供了另一种方式，使用Ingress和Ingress控制器来对外暴露服务，Ingress控制器作为统一的流量入口，管理内部各种必要的服务，并通过Ingress资源来描述如何区分流量及内部的路由逻辑。有了Ingress和Ingress控制器，就可以通过定义路由流量的规则来实现服务发布，而无需创建NodePort或LoadBalancer类型的Service，并且流量也会由Ingress控制器直达Pod，不需要再由Service转发。</p><div class="hint-container info"><p class="hint-container-title">相关信息</p><p>Ingress资源就是基于HTTP虚拟主机或URL路径的流量转发规则（类似于nginx中的虚拟主机定义或location转发规则定义），它把需要暴露给集群外的每个Service对象，映射为Ingress控制器上的一个虚拟主机或某虚拟主机的一个URL路径。</p></div><p>如下图所示：</p><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675821066302-e7f2e7a2-7032-4d52-b186-470babac7d1b.png#averageHue=%23f1dacd&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=452&amp;id=u548d2323&amp;name=image.png&amp;originHeight=452&amp;originWidth=611&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=27821&amp;status=done&amp;style=none&amp;taskId=uc2dbc2b0-ffb7-4d90-9d7b-c9f3fed1582&amp;title=&amp;width=611" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure>',7),u={href:"https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress/",target:"_blank",rel:"noopener noreferrer"},d=s("br",null,null,-1),m={href:"https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/",target:"_blank",rel:"noopener noreferrer"},k=t('<p><a name="JgTFh"></a></p><h3 id="ingress控制器" tabindex="-1"><a class="header-anchor" href="#ingress控制器" aria-hidden="true">#</a> Ingress控制器</h3><p>但Ingress资源本身只是一组路由规则定义，这些规则想要真正的生效还需要借助其它功能的辅助，例如监听某套接字、根据路由规则匹配机制将客户端请求进行转发等。实现这些功能的组件就是Ingress控制器(Ingress Controller)。Ingress Controller是Kubernetes的一个附件需要单独部署。</p><p><a name="AQ8zJ"></a></p><h2 id="ingress-controller部署" tabindex="-1"><a class="header-anchor" href="#ingress-controller部署" aria-hidden="true">#</a> Ingress Controller部署</h2>',5),v={href:"https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/",target:"_blank",rel:"noopener noreferrer"},b={href:"https://github.com/kubernetes/ingress-nginx",target:"_blank",rel:"noopener noreferrer"},g=s("br",null,null,-1),h={href:"https://kubernetes.github.io/ingress-nginx/",target:"_blank",rel:"noopener noreferrer"},y=t(`<p>** 常用的的Ingress控制器部署方式有两种： **</p><p><a name="Cd3df"></a></p><h3 id="_1-以deployment方式部署ingress控制器pod资源" tabindex="-1"><a class="header-anchor" href="#_1-以deployment方式部署ingress控制器pod资源" aria-hidden="true">#</a> 1. 以Deployment方式部署Ingress控制器Pod资源</h3><p>通过NodePort或LoadBalancer类型的Service或者通过拥有外部IP地址（externalIP）的Service对象为其接入集群外部的客户端请求流量。<br>这意味着，在生产环境以这种方式部署一个Ingress控制器时，必须在其前端定义一个负载均衡器，这个负载均衡器可以是LoadBalancer类型的Service，也可以是用户自行管理的负载均衡器。</p><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675822367503-e4adce9e-bfca-4bb3-9b2f-549eac4f60ed.png#averageHue=%23f8f2ef&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=614&amp;id=u440a438b&amp;name=image.png&amp;originHeight=614&amp;originWidth=733&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=45763&amp;status=done&amp;style=none&amp;taskId=uec696fd8-906b-4a0d-8682-a98c7ffc3f4&amp;title=&amp;width=733" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><p><a name="PTwIB"></a></p><h3 id="_2-以daemonset方式部署ingress控制器" tabindex="-1"><a class="header-anchor" href="#_2-以daemonset方式部署ingress控制器" aria-hidden="true">#</a> 2.以DaemonSet方式部署Ingress控制器</h3><p>Pod资源Ingress控制器的各Pod分别以单一实例的方式运行在集群的所有节点或部分专用节点之上，并配置这些Pod对象以hostPort或hostNetwork的方式在当前节点接入外部流量。在这种方式下，前端还是需要一个负载均衡器，作为客户端流量的统一入口，然后转发给Ingress控制器Pod</p><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675822438502-f95a9c45-e281-47e8-b65f-fce5abecd40a.png#averageHue=%23f6f6f2&amp;clientId=u61a03d31-46df-4&amp;from=paste&amp;height=486&amp;id=u9ac953d6&amp;name=image.png&amp;originHeight=486&amp;originWidth=637&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=28055&amp;status=done&amp;style=none&amp;taskId=ud1143d45-3b31-4158-96f6-1775ca6a71b&amp;title=&amp;width=637" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><p>在nginx Ingress控制器官方提供的部署文件中，默认使用第一种方式，使用Deployment+NodePort Service来部署。</p><p><a name="zJrih"></a></p><h3 id="_3-deployment方式部署" tabindex="-1"><a class="header-anchor" href="#_3-deployment方式部署" aria-hidden="true">#</a> 3. Deployment方式部署</h3><p>选定好版本，下载对应的部署文件</p><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1675833096495-64fbd3a8-e6ab-4eeb-9c19-063d0909ac07.png#averageHue=%23141a21&amp;clientId=ua68a97d2-e0ab-4&amp;from=paste&amp;height=613&amp;id=u1943c99c&amp;name=image.png&amp;originHeight=613&amp;originWidth=679&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=42264&amp;status=done&amp;style=none&amp;taskId=u507f2dba-06ee-406d-ac54-eb7941f0218&amp;title=&amp;width=679" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">wget</span> https://github.com/kubernetes/ingress-nginx/archive/refs/tags/controller-v1.3.1.tar.gz
...
tomcat app2 ingress
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><a name="sPXIg"></a></p><h4 id="_3-2-1-单域名-ingress-资源" tabindex="-1"><a class="header-anchor" href="#_3-2-1-单域名-ingress-资源" aria-hidden="true">#</a> 3.2.1 单域名 ingress 资源</h4>`,49),f={href:"http://xn--www-zr0fl02s60m.app1.com",target:"_blank",rel:"noopener noreferrer"},x=t(`<div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">apiVersion</span><span class="token punctuation">:</span> networking.k8s.io/v1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="https://cdn.nlark.com/yuque/0/2023/png/33538388/1676445008460-e809acd8-60e2-4ef6-ac23-1faa36ddf2cc.png#averageHue=%2312151e&amp;clientId=ub0ecf962-4f87-4&amp;from=paste&amp;height=698&amp;id=uf571f9a8&amp;name=image.png&amp;originHeight=698&amp;originWidth=1887&amp;originalType=binary&amp;ratio=1&amp;rotation=0&amp;showTitle=false&amp;size=266981&amp;status=done&amp;style=none&amp;taskId=u85c0173a-13ef-4387-bf95-bd584af672a&amp;title=&amp;width=1887" alt="image.png" tabindex="0" loading="lazy"><figcaption>image.png</figcaption></figure><p>和之前的访问结果进行对比，可以看到证书已经被更新</p>`,21);function I(S,P){const a=c("ExternalLinkIcon");return p(),l("div",null,[r,s("p",null,[n("Ingress官方文档："),s("a",u,[n("https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress/"),e(a)]),d,n("Ingress控制器官方文档："),s("a",m,[n("https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/"),e(a)])]),k,s("p",null,[n("目前可选择使用的Ingress控制器有很多，可以参考官方介绍："),s("a",v,[n("https://kubernetes.io/zh-cn/docs/concepts/services-networking/ingress-controllers/"),e(a)]),n(" ，下面以nginx Ingress控制器为例进行部署。")]),s("p",null,[n("nginx Ingress控制器github地址："),s("a",b,[n("https://github.com/kubernetes/ingress-nginx"),e(a)]),g,n("nginx Ingress控制器官方文档："),s("a",h,[n("https://kubernetes.github.io/ingress-nginx/"),e(a)])]),y,s("p",null,[s("a",f,[n("将访问www.app1.com"),e(a)]),n(" 域名的流量转发至tomcat-app1-svc，Ingress部署文件如下：")]),x,s("p",null,[s("a",w,[n("首先准备www.myapp1.com"),e(a)]),n(" 域名的证书，然后将证书保存为Secret")]),_])}const T=i(o,[["render",I],["__file","kubernetes-12.html.vue"]]);export{T as default};
