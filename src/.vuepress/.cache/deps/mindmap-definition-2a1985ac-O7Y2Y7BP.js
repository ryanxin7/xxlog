...
// node_modules/mermaid/dist/mindmap-definition-2a1985ac.js
...
var sanitizeText2 = (text) => sanitizeText(text, getConfig());
var nodes = [];
var cnt = 0;
var elements = {};
var clear = () => {
...
var addNode = (level, id, descr, type) => {
  log.info("addNode", level, id, descr, type);
  const conf = getConfig();
  const node = {
    id: cnt++,
    nodeId: sanitizeText2(id),
    level,
    descr: sanitizeText2(descr),
    type,
    children: [],
...
var setElementForId = (id, element) => {
  elements[id] = element;
};
var decorateNode = (decoration) => {
  const node = nodes[nodes.length - 1];
  if (decoration && decoration.icon) {
    node.icon = sanitizeText2(decoration.icon);
  }
  if (decoration && decoration.class) {
    node.class = sanitizeText2(decoration.class);
  }
};
var type2Str = (type) => {
  switch (type) {
...
var parseError;
var setErrorHandler = (handler) => {
  parseError = handler;
};
var getLogger = () => log;
var getNodeById = (id) => nodes[id];
var getElementById = (id) => elements[id];
var mindmapDb = Object.freeze(Object.defineProperty({
  __proto__: null,
  addNode,
  clear,
  decorateNode,
...
var rectBkg = function(elem, node) {
...
var cloudBkg = function(elem, node) {
  const w = node.width;
  const h = node.height;
  const r1 = 0.15 * w;
  const r2 = 0.25 * w;
...
var bangBkg = function(elem, node) {
  const w = node.width;
  const h = node.height;
  const r = 0.15 * w;
  elem.append("path").attr("id", "node-" + node.id).attr("class", "node-bkg node-" + type2Str(node.type)).attr(
...
var circleBkg = function(elem, node) {
...
var hexagonBkg = function(elem, node) {
  const h = node.height;
  const f = 4;
  const m = h / f;
...
var roundedRectBkg = function(elem, node) {
...
var svgDraw = { drawNode, positionNode, drawEdge };
var cytoscape;
...
async function layoutMindmap(node, conf) {
  if (!cytoscape) {
    cytoscape = (await import("./cytoscape.cjs-YF7EVSVR.js")).default;
    const coseBilkent = (await import("./cytoscape-cose-bilkent-UW6KWRMQ.js")).default;
...
var draw = async (text, id, version, diagObj) => {
  const conf = getConfig();
  diagObj.db.clear();
...
//# sourceMappingURL=mindmap-definition-2a1985ac-O7Y2Y7BP.js.map
